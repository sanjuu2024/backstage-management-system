...
> 以及如果有打算跟着这个视频学习该项目类型的朋友，以下是个人一点建议：
> （1）​前面一些工具的部署可以使用ai帮忙写配置项（视频版本和现在的一些配置写法不是特别匹配）；
> （2）官方接口不太好用，我自己是用的这个大佬的后端（github上）：WangJian3306/vue3_admin_backend，（wsl2+docker部署的）；
