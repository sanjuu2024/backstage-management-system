	...
	<el-card>
		<el-form label-width="auto" label-position="right">
			...
			<el-form-item label="SPU销售属性">
				<el-select
					:placeholder="
						unSelectedSaleAttr.length
							? `剩余${unSelectedSaleAttr.length}个可添加销售属性`
							: '无'
					"
					style="width: 200px"
					v-model="theSaleAttr"
				>
					<el-option
						v-for="(item, idx) in unSelectedSaleAttr"
						:key="idx"
						:label="item.name"
					...
					<el-table-column label="属性值" align="center">
						<template #="{ row, $index }">
							<el-tag
								...
								style="margin: 10px"
								closable
								@close="handleAttrValueClose($index, idx)"
							>
							...
							<el-input
								...
								size="small"
								width="100px"
								@blur="handleAttrValueInputBlur(row)"
...
async function init(row: SpuData) {
	...
	// 5.获取已有的spu销售属性值
	let res5: SpuSaleAttrListResponseData = await reqSpuSaleAttrList(
		row.id as number,
	);
	if (res5.code === 200) {
