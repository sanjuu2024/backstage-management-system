...
router.beforeEach(async (to:any,from:any,next:any) => {
	...
	// 已登录过
	if (token) {   // 🔺🔺🔺🍉🍉🍉要注意token是在userStore中的！而store的数据是缓存在内存中的！也就是，如下情况是正常的：(吗？对的。)如果现在正常使用，然后f12调出application删掉了token，但其实之后的点击菜单路由跳转是正常的，要直到下一次刷新的时候，路由守卫才会发现token已经没了 / 过期 / 失效 / 错误，才会强制跳转到/login。
		if (to.path === '/login') {
			...
			if (!userStore.userInfo.username) {
                try{
                    ...
                    alert('获取信息失败：'+err);
                    userStore.userLogout();   // token过期先退出登录，然后去到login界面
                    next({path:'/login'});
                }
			}
            else next();
