...
import type { AllBrandsResponseData, AllSaleAttrResponseData, SpuData, SpuImgListResponseData, SpuResponseData, SpuSaleAttrListResponseData } from "./type"
...
// 获取已有SPU分页列表
export const reqExistingSpu = async (page: number, limit: number, category3Id: string | number) => {
    return await request.get<any,SpuResponseData>(API.EXISTING_SPU_URL + `${page}/${limit}?category3Id=${category3Id}`);
}
...
export const reqAllSaleAttr = async () => {
    return await request.get<any,AllSaleAttrResponseData>(API.ALL_SALE_ATTR_URL);
}

// 获取某个SPU下所有商品图片
export const reqSpuImgList = async (spuId: number) => {
    return await request.get<any,SpuImgListResponseData>(API.IMAGE_URL + spuId);
}

// 获取某个SPU下所有销售属性
export const reqSpuSaleAttrList = async (spuId: number) => {
    return await request.get<any,SpuSaleAttrListResponseData>(API.SALE_ATTR_URL + spuId);
}
